...
(m, n) offline keys, where it is RECOMMENDED that n >= 2, and m >=
ceiling(n / 2). Utilizing such a threshold ensures that a majority of
the administrator keys are required to authorize changes to the supply
chain. A lower threshold means that fewer keys need to be compromised
to sign a malicious root layout. The administrators are responsible for
distributing, revoking, and rotating the developer keys used in the first
step. Alternatively, administrators MAY use a
...
it is RECOMMENDED that n >= 2, and m >= ceiling(n / 2), again setting a
requirement of consensus between a majority of the keys. Finally, its
...
from both the in-toto root layout and the targets role, once again
ensuring separation of roles. It is RECOMMENDED that n >= 2, and
m >= ceiling(n / 2), where the threshold again enforces a majority vote.
Its metadata SHOULD expire in 1 year. All TUF metadata and targets MUST be
written using
