...
expectedProducts: list of constraints # TODO: outcomes or products?
functionaries: list of strings
threshold: int
```

Each entry in `expectedMaterials` and `expectedProducts` corresponds to one
rule, similar to the current in-toto layouts. However, along with in-toto
artifact rules, other DSLs may be used to specify attribute specific
constraints. Languages such as rego, CUE, and CEL have been designed to enable
such schema and data validation. For more, see Appendix.

TODO: Appendix

Each constraint must identify the DSL used to ensure the verification engine can
appropriately handle the rules.
...
TODO: should inspections allow for assertions on specific subjects?
